...
function formatStylish(result) {
    ...
    targetResult.forEach(v => {
        ...
        v.diagnostics.forEach(v2 => {
            ...
            const messageType = v2.errorLevel === rules_1.ErrorLevel.Error ? chalk_1.default.red('error') : chalk_1.default.yellow('warning');
...
function lint(params) {
    ...
    const files = path ? utils_1.getFileList(path) : [];
    if (files.length === 0 && !text) {
        throw new Error(`No files matching. path: ${path}`);
    }
    const config = configObject ? loadConfig_1.convertToConfig(configObject) : loadConfig_1.loadConfig(configPath || process.cwd());
    let result = text
        ? [{ filepath: 'text', diagnostics: rules_1.execute(text, config) }]
        : files.map(v => {
            const diagnostics = rules_1.execute(utils_1.readFile(v), config);
    ...
    if (params.fix) {
        const fixedResult = result.map(v => {
            ...
            function fix(text, descriptions, loop = MAX_AUTOFIX_LOOP) {
                ...
                const fixed = fixer_1.applyFixes(text, descriptions);
                const nextDiagnostics = rules_1.execute(fixed, config);
            ...
            const fixedText = fix(params.text || utils_1.readFile(v.filepath), getFixDescriptions(v.diagnostics));
            const diagnostics = rules_1.execute(fixedText, config);
            return { filepath: v.filepath, diagnostics, fixedText };
        });
        if (!text) {
            fixedResult.forEach(v => {
                utils_1.writeFile(v.filepath, v.fixedText);
            });
        }
        result = fixedResult;
    }
    ...
    if (outputFile) {
        utils_1.writeFile(outputFile, output);
    }
    return output;
}
exports.lint = lint;
