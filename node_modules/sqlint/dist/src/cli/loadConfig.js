...
function validateSchema(config) {
    const ajv = new ajv_1.default({ verbose: true, schemaId: 'auto', missingRefs: 'ignore' });
...
function loadConfig(directoryOrFile) {
    let filePath = '';
    if (utils_1.fileExists(directoryOrFile)) {
        filePath = directoryOrFile;
    }
    else if (utils_1.directoryExists(directoryOrFile)) {
        const file = configFiles.find(v => utils_1.fileExists(`${directoryOrFile}/${v.name}`));
        if (file)
            filePath = `${directoryOrFile}/${file.name}`;
    }
    if (!filePath) {
        // try to lookup personal config file
        const file = configFiles.find(v => utils_1.fileExists(`${process.env.HOME}/.config/sql-language-server/${v.name}`));
        if (file)
    ...
    const fileContent = utils_1.readFile(filePath);
    let config;
