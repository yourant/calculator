...
function doLint(uri, sql, config) {
    ...
    const result = JSON.parse(sqlint_1.lint({ configPath: process.cwd(), formatType: 'json', text: sql, configObject: config }));
    ...
    const diagnostics = lintDiagnostics.map(v => {
        const diagnostic = {
            ...
            message: v.message,
            severity: v.errorLevel === sqlint_1.ErrorLevel.Error ? vscode_languageserver_types_1.DiagnosticSeverity.Error : vscode_languageserver_types_1.DiagnosticSeverity.Warning,
            source: 'sql',
            relatedInformation: []
        };
