...
class AbstractClient {
    ...
    async getSchema() {
        var _a, _b, _c;
        let schema = [];
        const sshConnection = ((_a = this.settings.ssh) === null || _a === void 0 ? void 0 : _a.remoteHost) ? new node_ssh_forward_1.SSHConnection({
            endHost: this.settings.ssh.remoteHost,
            username: this.settings.ssh.user,
            privateKey: fs_1.readFileSync(this.settings.ssh.identityFile || `${process.env.HOME}/.ssh/id_rsa`),
        ...
        try {
            ...
            schema = await Promise.all(tables.map((v) => this.getColumns(v).then(columns => ({
